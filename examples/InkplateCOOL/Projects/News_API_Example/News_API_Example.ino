/*
    Cryptocurrency tracker example for e-radionica.com Inkplate COOL
...
void setup()
{
    ...
    if (refreshes == 0)
    {
        ...
        display.println(F("Welcome to Inkplate COOL News API Example!"));
...
void drawNews(struct news *entities)
{
    ...
    while (coll <= 2)
    {
        display.setCursor(10 + 199 * coll, y);
        display.setFont(&GT_Pressura14pt7b);
        uint16_t cnt = 0;
        while (*(entities[i].title + cnt) != '\0')
        {
            if (display.getCursorX() > 179 * coll + 179 || (*(entities[i].title + cnt) == ' ' && display.getCursorX() > 179 * coll + 160))
            {
        ...
        {
            if (display.getCursorX() > 199 * coll + 179 || (*(entities[i].description + cnt) == ' ' && display.getCursorX() > 179 * coll + 160))
            {
